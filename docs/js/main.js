...
class Terminal extends BaseTerminal{
    ...
    */
    ...
    setFile() {
        return {
            command: (cmdComponents, rawData) => {
                let [command,filePath,...data] = cmdComponents
                rawData = rawData.replace(new RegExp(`${command} ${filePath}`), '')
                this.fs.setFile(filePath,rawData) ? this._unlock() : this._type('Not a valid file name');
            },
    ...
    whoami() {
        return {
            command: () => {
                this._type(`Username: ${this.config.user}
...
window.onload = new Terminal(document,config)._init();
